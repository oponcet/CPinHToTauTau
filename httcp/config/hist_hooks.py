...
def add_hist_hooks(config: od.Config) -> None:
    ...
    def fake_factor(task, hists):
        ...
        os_noniso_mc = hist_to_num(get_hist(mc_hist, "tautau_antiIso"), "tautau_antiIso_mc") # os_noniso
        os_noniso_data = hist_to_num(get_hist(data_hist, "tautau_antiIso"), "tautau_antiIso_data") # os_noniso
        ss_noniso_mc = hist_to_num(get_hist(mc_hist, "FFDRantiIso_tautau"), "FFDRantiIso_tautau_mc") # ss_noniso
        ss_iso_mc = hist_to_num(get_hist(mc_hist, "FFDRIso_tautau"), "FFDRIso_tautau_mc") # ss_iso
        ...
        # get pt edges : 
        pt_bin_edges = mc_hist.axes['tau_1_pt'].edges
        ...
        # plt.xlabel(r'p_T(\tau_h) [GeV]')  # Adjust x-axis label as necessary
        # plt.ylabel('Fake Factor')
        # plt.title(r'Fake Factor vs p_T(\tau_h) for \tau_h\tau_h')
        # # plt.grid(True)
        # plt.savefig(f"FakeFactors_tautau.png")  # save

        ## Save the data to a JSON file
        fake_factor_values_first = fake_factor_values[0]
        fake_factor_variances_first = fake_factor_variances[0]
        # Create a dictionary to store the data
        data_to_save = {
            "pt_centers": pt_centers.tolist(),  # Convert numpy array to list
            "fake_factor_values": fake_factor_values_first.tolist(),  # Convert to list
            "fake_factor_variances": fake_factor_variances_first.tolist()  # Convert to list
        }

        # Save to a JSON file
        with open('fake_factors_tautau.json', 'w') as json_file:
            json.dump(data_to_save, json_file, indent=4)

        print("Data saved to fake_factors.json")


        # ABCD method for the fake factor
        # shape: (SHIFT, VAR) 
        os_iso_qcd = os_noniso_qcd *  (ss_iso_qcd / ss_noniso_qcd)[:, None]# SR = AR * FF = AR * (ss_iso/ss_noniso)
        ...
        for cat_index in range(cat_axis.size):
            if cat_axis.value(cat_index) == qcd_groups["tautau"].id:
                factor_hist.view().value[cat_index, ...] = os_iso_qcd_values
                factor_hist.view().variance[cat_index, ...] = os_iso_qcd_variances
