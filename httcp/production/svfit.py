...
# from httcp.util import TetraVec, _invariant_mass, deltaR
...
@producer(
    uses={
        ...
        "hcandprod.pt", "hcandprod.eta", "hcandprod.phi", "hcandprod.mass","hcandprod.pdgId","MET.*",
    },
)
def svfit(self: Producer,
               events: ak.Array,
               P4_dict: dict, #P4_dict
               P4_gen_dict: dict, # P4_gen_dict
               **kwargs) -> tuple[ak.Array, dict]:
    ...
    # print(breakhere)
    print_if_verbose(verb, "events.fields : ", events.fields)
    ...
    # If channel_id equals 4 (tautau), decayType is assigned 1 tauhdecayType,
    ...
    decayType = ak.where(channel_id == 4, 1, ak.where(channel_id == 2, 2, ak.where(channel_id == 1, 3, channel_id)))
    ...
    # # Get hcand from P4_dict
    # print("P4_dict : ", P4_dict)
    # print("P4_dict fields : ", P4_dict["p4h1"].fields)
    # print("P4_dict fields : ", P4_dict["p4h2"].fields)
    # print("P4_gen_dict : ", P4_gen_dict)
    # print("P4_gen_dict fields : ", P4_gen_dict.fields)

    # Get the leptons pairs from the P4_dict
    leps1 = P4_dict["p4h1"] # e muon or tau
    leps2 = P4_dict["p4h2"] # always tau

    # Get the gen leptons pairs from the P4_gen_dict
    leps1_gen = P4_gen_dict["p4h1"] # e muon or tau
    leps2_gen = P4_gen_dict["p4h2"] # always tau
    ...
    leps1_dm = leps1["decayMode"]
    leps1_dm = ak.where(leps1_dm == -2, -1, leps1_dm)
    leps1_dm = ak.to_numpy(leps1["decayMode"])


    # GEN
    leps1_genpt = ak.to_numpy(leps1_gen["pt"])
    leps1_geneta = ak.to_numpy(leps1_gen["eta"]) 
    leps1_genphi = ak.to_numpy(leps1_gen["phi"])
    leps1_genmass = ak.to_numpy(leps1_gen["mass"])
    ...
    leps2_dm = leps2["decayMode"]
    leps2_dm = ak.where(leps2["decayMode"] == -2, -1, leps2["decayMode"])
    leps2_dm = ak.to_numpy(leps2["decayMode"])



    leps2_genpt = ak.to_numpy(leps2_gen["pt"])
    leps2_geneta = ak.to_numpy(leps2_gen["eta"])
    leps2_genphi = ak.to_numpy(leps2_gen["phi"])
    leps2_genmass = ak.to_numpy(leps2_gen["mass"])
    ...
    return events, {"p4h1_svfit"       : p4_hcand1,
                    "p4h2_svfit"       : p4_hcand2}
...
def fastMTT(self: Producer,
    ...
    leps2_mass = ak.to_numpy(leps2["mass"])
    leps2_dm = leps2["decayMode"]
    leps2_dm = ak.where(leps2_dm == -2, -1, leps2_dm)
    leps2_dm = ak.to_numpy(leps2["decayMode"])
    ...
    p4_hcand1 = ak.zip({"pt": tau1_pt_array, "eta": tau1_eta_array, "phi": tau1_phi_array, "mass": leps1_mass},
                        with_name="PtEtaPhiMLorentzVector",
                        behavior=coffea.nanoevents.methods.vector.behavior)

    p4_hcand2 = ak.zip({"pt": tau2_pt_array, "eta": tau2_eta_array, "phi": tau2_phi_array, "mass": leps2_mass},
                        with_name="PtEtaPhiMLorentzVector",
                        behavior=coffea.nanoevents.methods.vector.behavior)

    # return events
    return events, {"p4h1_fastmtt"       : p4_hcand1,
                    "p4h2_fastmtt"       : p4_hcand2}
