...
setup_httcp() {
    ...
    if [ "${HTTCP_SETUP}" = "1" ]; then
        >&2 echo "the CPinHToTauTau analysis was already succesfully setup"
    ...
    #
    # global variables
    # (HTTCP = CPinHToTauTau, CF = columnflow)
    #
...
main() {
    ...
    if setup_httcp "$@"; then
        cf_color green "CPinHToTauTau analysis successfully setup"
        return "0"
    else
        local code="$?"
