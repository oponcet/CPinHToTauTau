...
def process_histograms(base_path, task, datasets_data, datasets_mc, hist_path, category, cats_title):
    ...
    hists_mc_data_sub = {}
    # print(f"hists_data = {hists_data}")
    # print(f"hists_mc = {hists_mc}")

    # Define the new binning    
    
    for cat, cat_title in zip(category, cats_title):
        # print(f"Processing category: {cat} - {cat_title}")
        hists_data_cat, hists_mc_cat = get_hist_by_category_var(hists_data, hists_mc, cat,var = 'hcand_1_pt')
        # print(f"Data histograms: {hists_data_cat}")
        # print(f"MC histograms: {hists_mc_cat}")
        
        # If empty, skip the function
        if not hists_data_cat or not hists_mc_cat:
            print(f"Skipping because of empty histograms of category {cat} - {cat_title}") 
            return None
...
def main(config_path):
    ...
    for dm_name, dm_categories in categories.items():
        ...
        for njets_name, njets_categories in dm_categories.items():
            print(f"Processing njets category {njets_name}")
        # ABCD or ABCD0
            for grouping_name, category_dict in njets_categories.items(): 
                ...
                if grouping_name == "ABCD":
                    ...
                    catA, catB, catC, catD = [category_dict[key] for key in category_dict if 'hadA' in key][0], \
                                             [category_dict[key] for key in category_dict if 'hadB' in key][0], \
                                             [category_dict[key] for key in category_dict if 'hadC' in key][0], \
                                             [category_dict[key] for key in category_dict if 'hadD' in key][0]
                    print(f"catA: {catA}")
                    print(f"catB: {catB}")
                    print(f"catC: {catC}")
                    print(f"catD: {catD}")
                    ...
                    catA, catB, catC, catD = [category_dict[key] for key in category_dict if 'hadA0' in key][0], \
                                             [category_dict[key] for key in category_dict if 'hadB0' in key][0], \
                ...
                # if hists_mc_data_sub_ABC is empty skip the njets category
                if not hists_mc_data_sub_ABC:
                    continue
